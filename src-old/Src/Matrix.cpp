/*!
 * \file Matrix.cpp
 * \brief Class for matrix arithmetic.
 * \author D. Monett Diaz
 * \date November, 2006
 * 
 * This class implements some functions used when working with matrices. 
 * They are used in the index determination in systems of ordinary differential
...
template <typename T> Matrix<T>::Matrix()
{
	try
	{
		...
		setmaxdim( 1, 1 );
		_data = new T*[ 1 ];								// data allocation
		if( (_data == 0)  ||  (_data == NULL))
			throw IDException( "Memory allocation failure.", 3 );
		_data[ 0 ] = new T[ 1 ];
		if( (_data[ 0 ] == 0)  ||  (_data[ 0 ] == NULL))
			throw IDException( "Memory allocation failure.", 3 );
		if( strcmp( _data[ 0 ][ 0 ].typeName(), "TPolyn" ) == 0 )// A zero value indicates 
															// that both strings are equal
			_data[ 0 ][ 0 ].set2zero();						// p(x) = 0, some initialization
		//else _data[ 0 ][ 0 ] = 0.0;
	}
	catch( bad_alloc e )
	{
 ...
 * 
 */
template <typename T> Matrix<T>::Matrix( int r, int c ) : _rows ( r ), _cols ( c )
{
	try
	{
 ...
 * 
 */
template <typename T> Matrix<T>::Matrix( int r, int c, int dimT ) 
 ...
 * Copy constructor.
 * 
 * 		Matrix<T> *newm = new Matrix<T>( (*m) );
 * 
 * \param[in] m A pointer to the \a Matrix<T> object to copy from.
 * 
 */
template <typename T> Matrix<T>::Matrix( const Matrix<T> &m )
{
	...
	try
	{
		...
		_data = new T*[ m._rows ];							// data allocation
		if( (_data == 0)  ||  (_data == NULL))
			throw IDException( "Memory allocation failure.", 3 );
		for( i = 0; i < m._rows; i++ )
		{
			_data[ i ] = new T[ m._cols ];
			if( (_data[ i ] == 0)  ||  (_data[ i ] == NULL) )
				throw IDException( "Memory allocation failure.", 3 );
 ...
 * 
 */
template <typename T> Matrix<T> Matrix<T>::redim( int r, int c )
{
	int i, j;
 ...
 * 
 */
template <typename T> Matrix<T> Matrix<T>::redim( int r, int c, int dimT )
{
	int i, j;
 ...
 * 
 * (T & Matrix<T>::... with '&' allows m(2] = 7, i.e., '( )' also in the left side!!)
 * 
 ...
 * 		newm = m;
 * 
 * \param[in] m The \a Matrix<T> object to assign to.
 * \return A pointer to the \a Matrix<T> new object.
 * 
 */
 ...
 * 
 */
template <typename T> bool Matrix<T>::operator==( const Matrix<T> &m )
{
	bool equal = true;
 ...
 * 
 * \param[in] m The \a Matrix<T> object to add up.
 * \return A pointer to the resulting \a Matrix<T> object.
 * 
 */
template <typename T> Matrix<T> Matrix<T>::operator+( const Matrix<T> &m )
{	
	Matrix<T> aux( _rows, _cols, _dimT );					// an auxiliary object
 ...
 * Implements the += operator. It adds up two matrices.
 * 
 * \param[in] m The \a Matrix<T> object to add up to the current matrix.
 * \return A pointer to the resulting \a Matrix<T> object.
 * 
 */
template <typename T> Matrix<T> Matrix<T>::operator+=( const Matrix<T> &m )
 ...
 * 
 * \param[in] m The \a Matrix<T> object to substract from.
 * \return A pointer to the resulting \a Matrix<T> object.
 * 
 */
template <typename T> Matrix<T> Matrix<T>::operator-( const Matrix<T> &m )
 ...
 * 
 */
template <typename T> Matrix<T> Matrix<T>::operator*( double alpha )
{
	Matrix<T> aux( _rows, _cols, _dimT );					// an auxiliary object
 ...
 * \param[in] A The pointer to \a A, an object of type \type Matrix<T>.
 * \param[in] B The pointer to \a B, an object of type \type Matrix<T>.
 * \return The error code.
 * 
 */
template <typename T> int Matrix<T>::mmCaABbC( double alpha, double beta, Matrix<T> &A, Matrix<T> &B )
{
	try
	{
 ...
 *
 */
template <typename T> int Matrix<T>::bmmCaABbC( int r, int c, double alpha, double beta, 
 ...
 *
 */
template <typename T> int Matrix<T>::mmCasABbC( int r, double alpha, double beta, Matrix<T> &A, Matrix<T> &B )
{
	int i, j, k;
 ...
 *
 */
template <typename T> int Matrix<T>::mmCaAsBbC( int r, double alpha, double beta, Matrix<T> &A, Matrix<T> &B )
{
	int i, j, k;
 ...
 * \param[in] beta The scalar value that multiplies \a C.
 * \param[in] A The pointer to \a A, an object of type \type Matrix<T>.
 * \param[in] B The pointer to \a B, an object of type \type Matrix<T>. Only its upper triangular 
 * 		part is interesting.
 * \param[in] piv The pointer to \a piv, a vector of permutations on the columns of \a A.
 * \return The error code.
 *
 */
template <typename T> int Matrix<T>::mmCaAUTBPbC( double alpha, double beta, Matrix<T> &A, 
		Matrix<T> &B, int *piv )
{
	try
	{
 ...
 * \param[in] alpha The scalar value that multiplies \a A*B.
 * \param[in] beta The scalar value that multiplies \a C.
 * \param[in] A The pointer to \a A, an object of type \type Matrix<T>. Its transpose is also considered.
 * \return The error code.
 *
 */
template <typename T> int Matrix<T>::mmCaAATbC( double alpha, double beta, Matrix<T> &A )
{
	try
	{
		for( int i = 0; i < A.nrows(); i++ )
 ...
 *
 */
template <typename T> int Matrix<T>::mmCaATAbC( double alpha, double beta, Matrix<T> &A )
{
	try
	{
		for( int i = 0; i < A.nrows(); i++ )
 ...
 * \param[in] alpha The scalar value that multiplies \a A*B.
 * \param[in] beta The scalar value that multiplies \a C.
 * \param[in] A The pointer to \a A, an object of type \type Matrix<T>. Its transpose is considered.
 * \param[in] B The pointer to \a B, an object of type \type Matrix<T>.
 * \return The error code.
 *
 */
template <typename T> int Matrix<T>::mmCaATBbC( double alpha, double beta, Matrix<T> &A, Matrix<T> &B )
{
	try
	{
		for( int i = 0; i < A.nrows(); i++ )
 ...
 * 
 */
template <typename T> int Matrix<T>::mmCaATBPbC( double alpha, double beta, Matrix<T> &A, 
		Matrix<T> &B, int *piv )
{
	try
	{
 ...
 * \param[in] beta The scalar value that multiplies \a C.
 * \param[in] A The pointer to \a A, an object of type \type Matrix<T>.
 * \param[in] B The pointer to \a B, an object of type \type Matrix<T>. Its transpose is considered.
 * \return The error code.
 * 
 */
template <typename T> int Matrix<T>::mmCaABTbC( double alpha, double beta, Matrix<T> &A, Matrix<T> &B )
{
	try
	{
		for( int i = 0; i < A.nrows(); i++ )
 ...
 * 
 */
template <typename T> int Matrix<T>::mmCaABTbC( int r, bool up, double alpha, double beta, 
		Matrix<T> &A, Matrix<T> &B )
{
	try
	{
 ...
 * 
 */
template <typename T> int Matrix<T>::bmmCaABTbC( int r, int c, double alpha, double beta, 
		Matrix<T> &A, Matrix<T> &B )
{
	int i, j, k;
 ...
 * 
 */
template <typename T> int Matrix<T>::mmCaIBbC( double alpha, double beta, Matrix<T> &B )
{
	int i, j;
 ...
 * 
 */
template <typename T> int Matrix<T>::mmCaIBbC( double alpha, double beta, int *piv, bool rows, Matrix<T> &B )
{
	int i, j;
	
	try
	{
		Matrix<T> Id( nrows(), B.nrows(), dimT() );			// Id[m][p][nTcoeff]
 ...
 * 
 */
template <typename T> int Matrix<T>::mmCaAIbC( double alpha, double beta, Matrix<T> &A )
{
	int i, j;
 ...
 * 
 */
template <typename T> int Matrix<T>::mmCaAIbC( double alpha, double beta, Matrix<T> &A, int *piv, bool rows )
{
	int i, j;
	
	try
	{
		Matrix<T> Id( A.ncols(), ncols(), dimT() );			// Id[p][n][nTcoeff]
		Id.set2Id();										// set to the identity
		if( rows )											// permute its columns
			Id.cpermutem( piv );
		else												// permute its rows
 ...
 * 
 * \param[in/out] B The pointer to \a B, an object of type \type Matrix<T> that is the independent 
 * 		term on input. On output it contains the calculated solution \a X.
 * \return The error code.
 * 
 */
template <typename T> int Matrix<T>::utsolve( Matrix<T> &B )
{
 ...
 * 
 */
template <typename T> int Matrix<T>::utsolve( Matrix<T> &B, Matrix<T> &X, int *piv )
{
	T sum( dimT() );
 ...
 * 
 */
template <typename T> int Matrix<T>::utsolve( T *b )
{
	T sum( dimT() );
 ...
 * 
 */
template <typename T> int Matrix<T>::utxsolve( Matrix<T> &B )
{
	T sum( dimT() );
 ...
 * 
 */
template <typename T> int Matrix<T>::gsolve( Matrix<T> &B )
{
	int h, i, j, k, jpiv, jindex, *index;
	double scalemax = 0.0, ratiomax, ratio, *scale;
	T coeff( dimT() );
	Matrix<T> X( B.nrows(), B.ncols(), B.dimT() );
 ...
 * 
 */
template <typename T> int Matrix<T>::gsolve( T *b )
{
 ...
 * 
 */
template <typename T> double Matrix<T>::fnorm()
{
	double sum = 0.0;
 ...
 * 
 */
template <typename T> int Matrix<T>::tcfnorm( int nrTC, double *fn )
{
	double sum;
 ...
 * 
 */
template <typename T> int Matrix<T>::colnorm( double *c )
{
	try
	{
		for( int j = 0; j < _cols; j++ )
 ...
 * 
 */
template <typename T> int Matrix<T>::colnormdown( int pos, int *piv, double *c )
{
	try
	{
 ...
 * 
 */
template <typename T> int Matrix<T>::cpermutem( int *piv )
{
	int i, j;
	Matrix<T> aux( _rows, _cols, _dimT );					// auxiliary matrix
 ...
 * 
 */
template <typename T> int Matrix<T>::cpermutem( int *piv, bool trans )
{
	int i, j;
	Matrix<T> aux( _rows, _cols, _dimT );						// auxiliary matrix
 ...
 * 
 */
template <typename T> int Matrix<T>::rpermutem( int *piv )
{
	int i, j;
	Matrix<T> aux( _rows, _cols, _dimT );						// auxiliary matrix
 ...
 * 
 */
template <typename T> Matrix<T> Matrix<T>::transpm()
{
	Matrix<T> aux( _cols, _rows, _dimT );						// an auxiliary object
 ...
 * 
 */
template <typename T> int Matrix<T>::shift( Matrix<T> &M )
{	
	try
	{
 ...
 * 
 */
template <typename T> bool Matrix<T>::isId()
{
	bool id = true;
 ...
 * 
 */
template <typename T> bool Matrix<T>::isId( double eps )
{
	bool id = true;
 ...
 * 
 */
template <typename T> bool Matrix<T>::isId( int m1, int m2, int n1, int n2, double eps )
{
	bool id = true;
 ...
 * 
 */
template <typename T> bool Matrix<T>::isZero( double eps )
{
	bool z = true;
 ...
 * 
 */
template <typename T> int Matrix<T>::set2Id()
{
	for( int i = 0; i < _rows; i++ )
		for( int j = 0; j < _cols; j++ )
		{
 ...
 * 
 */
template <typename T> int Matrix<T>::set2Id( int m, int n )
{
	for( int i = 0; i < m; i++ )
		for( int j = 0; j < n; j++ )
		{
 ...
 * 
 */
template <typename T> int Matrix<T>::set2Id( int m1, int m2, int n1, int n2 )
{
	for( int i = m1; i < m2; i++ )
		for( int j = n1; j < n2; j++ )
		{
 ...
 * 
 */
template <typename T> int Matrix<T>::set2zero()
{
	set2val( 0.0 );
	return 0;
}
 ...
 * 
 */
template <typename T> int Matrix<T>::set2zero( int m, int n )
{
	for( int i = 0; i < m; i++ )
		for( int j = 0; j < n; j++ )
			if( strcmp( _data[ i ][ j ].typeName(), "TPolyn" ) == 0 ) // A zero value indicates 
 ...
 * 
 */
template <typename T> int Matrix<T>::set2zero( int m1, int m2, int n1, int n2 )
{
	for( int i = m1; i < m2; i++ )
		for( int j = n1; j < n2; j++ )
			if( strcmp( _data[ i ][ j ].typeName(), "TPolyn" ) == 0 ) // A zero value indicates 
 ...
 * 
 */
template <typename T> int Matrix<T>::set2val( double v )
{
	if( v == 0.0 )
	{
		for( int i = 0; i < _rows; i++ )
 ...
 * 
 */
template <typename T> int Matrix<T>::set2val( int i, int j, double v )
{
	if( v == 0.0 )
	{
		if( strcmp( _data[ i ][ j ].typeName(), "TPolyn" ) == 0 ) // A zero value indicates 
 ...
 * 
 * \param[out] Am The pointer to \a Am, an object of type \type Matrix<T> with the inverse of \a A. 
 * \return The error code.
 * 
 */
template <typename T> int Matrix<T>::trinvm( Matrix<T> &Am )
{
	int i, j, k;
	T sum( dimT() ), p( dimT() );
 ...
 * 
 */
template <typename T> int Matrix<T>::trinvm( int r, Matrix<T> &Am )
{
	int i, j, k;
	T sum( dimT() ), p( dimT() );
 ...
 * 
 * \param[in] B The pointer to \a B, an object of type \type Matrix<T> to compare to. 
 * \return \a true if the matrices are equal; \a false otherwise.
 * 
 */
template <typename T> bool Matrix<T>::mcompare( Matrix<T> &B )
{
	bool equal = true;
 ...
 * 
 */
template <typename T> bool Matrix<T>::mcompare( Matrix<T> &B, double eps )
{
	bool equal = true;
	T diff( dimT() );
 ...
 * 
 */
template <typename T> bool Matrix<T>::mcompare( Matrix<T> &B, int r, double eps )
{
	bool equal = true;
	T diff( dimT() );
 ...
 * 
 */
template <typename T> void Matrix<T>::printm( char *str )
{
	printf( yellow );
	printf( str );
	for( int i = 0; i < _rows; i++ )
 ...
 * 
 */
template <typename T> void Matrix<T>::fprintm( FILE * fn, char *str )
{
	fprintf( fn, str );
	for( int i = 0; i < _rows; i++ )
	{
 ...
 * 
 */
template <typename T> void Matrix<T>::printm( char *str, const char *const color )
{
	printf( color );
	printf( str );
	for( int i = 0; i < _rows; i++ )
 ...
 * 
 */
template <typename T> void Matrix<T>::printdm( char *str )
{
	printf( str );
	for( int i = 0; i < _rows; i++ )
	{
 ...
 * 
 */
template <typename T> void Matrix<T>::fprintdm( FILE * fn, char *str )
{
	fprintf( fn, str );
	for( int i = 0; i < _rows; i++ )
	{
 ...
 * 
 */
template <typename T> void Matrix<T>::printdm( char *str, const char *const color )
{
	printf( color );
	printf( str );
	for( int i = 0; i < _rows; i++ )
 ...
 * 
 */
template <typename T> void Matrix<T>::printdm( char *str, const char *const color, double eps )
{
	printf( color );
	printf( str );
	for( int i = 0; i < _rows; i++ )
 ...
 * 
 */
template <typename T> void Matrix<T>::fprintdm( FILE * fn, char *str, double eps )
{
	fprintf( fn, str );
	for( int i = 0; i < _rows; i++ )
	{
 ...
 * 
 */
template <typename T> void Matrix<T>::printm( int *piv, char *str, const char *const color )
{
	printf( color );
	printf( str );
	for( int i = 0; i < _rows; i++ )
 ...
 * 
 */
template <typename T> void Matrix<T>::printdm( int *piv, char *str, 
 ...
 * 
 */
template <typename T> void Matrix<T>::fprintm( FILE * fn, int *piv, char *str )
{
	fprintf( fn, str );
	for( int i = 0; i < _rows; i++ )
	{
 ...
 * 
 */
template <typename T> void Matrix<T>::fprintdm( FILE * fn, int *piv, char *str, double eps )
{
	fprintf( fn, str );
	for( int i = 0; i < _rows; i++ )
	{
 ...
 * 
 */
template <typename T> void Matrix<T>::printtpm( char *str, const char *const color, double eps )
{
 ...
 * 
 */
template <typename T> void Matrix<T>::fprinttpm( FILE * fn, char *str, double eps )
{
	int _coeff = _data[ 0 ][ 0 ].nrcoeff();
	fprintf( fn, str );
	for( int k = 0; k < _coeff; k++ )
 ...
 * 
 */
template <typename T> void Matrix<T>::printtpm( int m1, int m2, int n1, int n2,
 ...
 * 
 */
template <typename T> void Matrix<T>::fprinttpm( FILE * fn, int m1, int m2, int n1, int n2,
		char *str, double eps )
{
	int _coeff = _data[ 0 ][ 0 ].nrcoeff();
	fprintf( fn, str );
 ...
 * 
 */
template <typename T> void Matrix<T>::printtpm( int *piv, char *str, 
	const char *const color, double eps )
{
	int _coeff = _data[ 0 ][ 0 ].nrcoeff();
	printf( color );
 ...
 * 
 */
template <typename T> void Matrix<T>::fprinttpm( FILE * fn, int *piv, char *str, double eps )
{
	int _coeff = _data[ 0 ][ 0 ].nrcoeff();
	fprintf( fn, str );
	for( int k = 0; k < _coeff; k++ )
